...
// Para funcionar: Vá em Arquivo > Compartilhar > Publicar na Web > Escolha 'Valores separados por vírgula (.csv)'
const SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTn3hv21CFpbF54z68UzUKH3iCgVTo-bm0FEupBvp1re8lrgns2hbh4WkIjCNfAFOTB5EvyPXLvDZOs/pub?gid=0&single=true&output=csv";
    ...
    return (
        <motion.div
            ...
            whileHover={{
                y: -12,
                scale: 1.02,
                boxShadow: "0 30px 60px -12px rgba(197, 160, 89, 0.25), 0 0 0 1px rgba(197, 160, 89, 0.3)"
            }}
            className="rounded-3xl p-8 bg-white flex-shrink-0 w-[300px] md:w-[360px] flex flex-col justify-between"
            style={{
                border: "1px solid rgba(197, 160, 89, 0.15)",
                boxShadow: "0 10px 30px rgba(0,0,0,0.03)",
                pointerEvents: "auto" // Garante que o hover funcione mesmo dentro do drag
            }}
        >
            <div>
                <QuoteIcon />
                ...
                <p
                    className="text-base leading-relaxed mb-8 text-zinc-600 font-light"
...
export default function TestimonialsSection() {
    ...
    useEffect(() => {
        const fetchTestimonials = async () => {
            try {
                ...
                // Parser robusto para CSV (lida com aspas e vírgulas internas)
    ...
    // Atualiza limites do drag
    useEffect(() => {
        const updateConstraints = () => {
            if (carouselRef.current && trackRef.current) {
                const trackWidth = trackRef.current.scrollWidth;
                const carouselWidth = carouselRef.current.offsetWidth;
                const leftConstraint = trackWidth > carouselWidth ? -(trackWidth - carouselWidth) : 0;
                setDragConstraints({ left: leftConstraint, right: 0 });
            }
        };

        // Pequeno delay para garantir que o DOM renderizou os novos cards do fetch
    ...
    }, [testimonials]); // Re-calcula quando os depoimentos mudarem

    if (testimonials.length === 0) return null;

    return (
        <div className="min-h-screen flex flex-col items-center justify-center py-24 bg-[#F5F5F4] relative overflow-hidden" id="depoimentos">

            {/* Ambient glow da identidade visual */}
            ...
            <div
                ref={carouselRef}
                className="w-full max-w-[1400px] mx-auto overflow-hidden px-6 md:px-12 py-10"
            >
                <motion.div
                    ref={trackRef}
                    drag="x"
